...
class LINQ
  ...
  # Selects
  Where: (fn) -> new LINQ (item for item, idx in @items when fn(item, idx) is true)
  Distinct: -> new LINQ (item for item in @items when !(item in _results))
  Except: (arr, fn) -> @Where (item) -> !(item in arr)
  ...
  Any: (fn) ->
    return true for item, idx in @items when fn(item, idx) is true
    return false
  All: (fn) ->
    return false for item, idx in @items when fn(item, idx) is false
    return true
